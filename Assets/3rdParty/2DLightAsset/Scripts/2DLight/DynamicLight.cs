...
public class DynamicLight : MonoBehaviour {
    ...
    [HideInInspector]public List<Vector2[]> allPaths;//MIKE
		...
		//for (int m = 0; m < 1; m++) {
            ...
            for(int p = 0; p < mf.pathCount; p++)
            {
                ...
                if (((1 << mf.transform.gameObject.layer) & layer) != 0)
                {
                    ...
                    {                                  // ...and for ever vertex we have of each mesh filter... totalPointCount wont work for my maps. - Mike

                        verts v = new verts();
                        // Convert to world space
                        Vector3 worldPoint = mf.transform.TransformPoint(path[i]);
