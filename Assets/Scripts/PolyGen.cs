...
public static class PolyGen
{
        ...
        for (int py = cy; py < cy + MapChunk.chunkSize; py++)
        {
            ...
            for (int px = cx; px < cx + MapChunk.chunkSize; px++)
            {
                ...
                foreach (string layername in MapGenerator.instance.Map.layer.Keys)
                {
                    ...
                    int? tID = ResourceManager.tileIdToTilesetId(map[py, px]);
                    ...
                    TilesetInfo info = ResourceManager.tilesetInfo[(int)tID] as TilesetInfo;
                    curSubLayerName = info.name;
...

                    {
                        ...
                        if (chunk.layers[layername].Collisions)
                        {
                            ...
                            List<IntPoint> ColliderInfo = ResourceManager.getTileColliderInfo(map[py, px]);
    ...
    {
        int? idExists = ResourceManager.globalIdToLocalId(id);
        if (idExists != null)
        {
            id = (int)idExists;
        }
